...
1. **RP2350 bootable packaging.** All the RP2350 code (armv8-m.main) *compiles and
   links* with clang against the armv8-m picolibc/compiler-rt + the same
   ArduinoCore-API (verified: valid `EM_ARM` ELF, 94 ArduinoCore-API symbols).
   But a *bootable* RP2350 image needs the SDK's embedded **IMAGE_DEF block**
   (`0xffffded3` marker + item `0x42`, block-loop) that the RP2350 bootrom
   requires; that block is generated by the SDK/picotool and is not yet
   hand-produced/hardware-verified here. RP2350 boot2 is also different from
   RP2040's.
2. **Full pico-sdk build with a stock clang** (the `pico_arm_cortex_m*_clang`
